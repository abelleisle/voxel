...
void render(){
	...
	**************************/
	...
	auto v = c.verts;
		...
		glVertexPointer(3,GL_FLOAT,0,&c.verts[0]);
		//glColorPointer(3,GL_FLOAT,6*sizeof(float),&f[3]);
		glDrawElements(GL_QUADS,c.vertOrder.size(),GL_UNSIGNED_INT,&c.vertOrder[0]);
